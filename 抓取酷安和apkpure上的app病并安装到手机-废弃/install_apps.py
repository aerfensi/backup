...
class Apps():
    def read_csv(self, mode, down_only):
        apps = {}
        if not down_only:
    ...
    def get(self):
        ...
        with self._condition:
            if not self.count(AppState.DOWNLOADING, AppState.DOWNLOADED):
                ...
                return
            # 无DOWNLOADED的apk，有DOWNLOADING的apk，所以此处要wait
    ...
    def __init__(self, mode, down_only):
        self.read_csv(mode, down_only)
...
def install(apps, rm):
    while True:
        ...
        try:
            ...
            # TODO: 安装失败后用aapt判断apk的完整性，如果是因为apk没下载完整导致安装失败，修改app state，并提交任务到线程池
        else:
            print('安装成功', pkg)
...
def main():
    ...
    # TODO: 为了让其他地方也能提交任务到线程池，不要使用with语句
