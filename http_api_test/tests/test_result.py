...
class TestCheckResult(TestCase):
    ...
    def test_get_by_jsonpath(self):
        checkpoint='$.id==12345'
        illusts_len = result.get_by_jsonpath(checkpoint, self.json_str)
    ...
    def test_check(self):
        checkpoint='$.id==1\n$.name=="黄狗"'
        json_str={"id":None,"category":{"id":0,"name":"狗"},"name":"黄狗","photoUrls":["string"],"tags":[{"id":1,"name":"黄色"}],"status":"available"}
