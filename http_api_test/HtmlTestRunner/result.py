...
def strip_module_names(testcase_names):
    """Examine all given tests case names and strip them the minimal
    names needed to distinguish each. This prevents cases where tests
...
class _TestInfo(object):
    """" Keeps information about the execution of a tests method. """
...
class HtmlTestResult(TextTestResult):
    """ A tests result class that express tests results in Html. """
    ...
    def _prepare_callback(self, test_info, target_list, verbose_str,
        ...
        def callback():
            """ Print tests method outcome to the stream and elapsed time too."""
    ...
    def getDescription(self, test):
        """ Return the tests description if not have tests name. """
        return str(test)
    ...
    def stopTest(self, test):
        """ Called after excute each tests method. """
    ...
    def _get_info_by_testcase(self):
        """ Organize tests results by TestCase module. """
        ...
        # unittest tests in alphabetical order based on tests name so re-assert this
    ...
    def _get_report_summaries(self, all_results, testRunner):
        """ Generate headers and summaries for all given tests cases."""
        summaries = {}
    ...
    def generate_reports(self, testRunner):
        """ Generate reports(s) for all given tests cases that have been run. """
        ...
        if not testRunner.combine_reports:
            for test_case_class_name, test_case_tests in all_results.items():
                ...
                # append tests case name if multiple reports to be generated
            ...
            # if available, use user reports name
    ...
    def generate_file(self, testRunner, report_name, report):
        """ Generate the reports file in the given path. """
    ...
    def _exc_info_to_string(self, err, test):
        ...
        if self.buffer:
            ...
            # StringIO yet, e.g. when tests fails during __call__
            try:
